    ...
    >
{
    ...
    CustomCameraHelper *cameraHelper;
...
@implementation CameraCustomPushVC

- (instancetype)init {
    if (self = [super init]) {
        _appIsInActive = NO;
        _appIsBackground = NO;
        cameraHelper = [[CustomCameraHelper alloc]init];
        [cameraHelper checkPermission];
...
#pragma mark - custom camera capture data call back
-(void)onVideoSampleBuffer:(CMSampleBufferRef)videoBuffer{
    [self.pusher sendVideoSampleBuffer:videoBuffer];
}
